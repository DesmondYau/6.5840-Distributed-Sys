...
/**
 * @brief Helper to escape strings for JSON and HTML embedding.
 */
...
static const char* porcupine_html_template = R"HTML(
<!DOCTYPE html>
<html>
...
std::string ComputeVisualizationJson(const Model& model, const LinearizationInfo& info) {
    ...
    json << "["; // Start of visualizationData array

    for (size_t p = 0; p < info.history.size(); ++p) {
        if (p > 0) json << ",";
        json << "{";

        // 1. History Reconstruction
        json << "\"History\": [";
        const auto& hist = info.history[p];
        std::map<int, std::any> callValues;
        std::map<int, std::any> returnValues;
        
        // Match paired calls/returns to generate descriptions
        std::map<int, size_t> idToIndex;
        struct HistoryElement { int clientId; int64_t start, end; std::string desc; };
        std::vector<HistoryElement> elements(hist.size() / 2);

        int count = 0;
        for (const auto& entry : hist) {
            if (entry.kind == EntryKind::Call) {
                elements[entry.id].clientId = entry.clientId;
                elements[entry.id].start = entry.time;
                callValues[entry.id] = entry.value;
            } else {
                elements[entry.id].end = entry.time;
                returnValues[entry.id] = entry.value;
                elements[entry.id].desc = model.DescribeOperation(callValues[entry.id], entry.value);
            }
        }

        for (size_t i = 0; i < elements.size(); ++i) {
            if (i > 0) json << ",";
            json << "{\"ClientId\":" << elements[i].clientId 
                 << ",\"Start\":" << elements[i].start 
                 << ",\"End\":" << elements[i].end 
                 << ",\"Description\":\"" << json_escape(elements[i].desc) << "\"}";
        }
        json << "],";

        // 2. Partial Linearizations
        json << "\"PartialLinearizations\": [";
        std::map<int, int> largestIndex;
        std::map<int, size_t> largestSize;

        const auto& partials = info.partialLinearizations[p];
        for (size_t i = 0; i < partials.size(); ++i) {
            ...
            for (size_t j = 0; j < partials[i].size(); ++j) {
                if (j > 0) json << ",";
                int histId = partials[i][j];

                auto [ok, nextState] = model.Step(state, callValues[histId], returnValues[histId]); 
                if (!ok) {
                    // This should not happen if the checker returned this as a valid partial
                    break;
                }
                
                state = nextState;
                json << "{\"Index\":" << histId 
                     << ",\"StateDescription\":\"" << json_escape(model.DescribeState(state)) << "\"}";
                
                if (partials[i].size() > largestSize[histId]) {
                    largestSize[histId] = partials[i].size();
        ...
        json << "\"Largest\": {";
        bool first = true;
        for (auto const& [id, idx] : largestIndex) {
            if (!first) json << ",";
            json << "\"" << id << "\":" << idx;
            first = false;
        }
...
/**
 * @brief Embeds the JSON data into the HTML template from visualization.go.
 */
void Visualize(const Model& model, const LinearizationInfo& info, std::ostream& out) {
   
    std::string jsonData = ComputeVisualizationJson(model, info);

    std::string html = porcupine_html_template;
    
    // Simple replacement of %s in the template
    size_t pos = html.find("%s");
    if (pos != std::string::npos) {
        html.replace(pos, 2, jsonData);
    }
